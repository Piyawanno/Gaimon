<div class="abstract_input_box input_per_line_{{{inputPerLine}}} full {{#isHidden}}hidden{{/isHidden}}" rel="{{{columnName}}}_box">
	...
	<div><textarea rel="{{{columnName}}}" autocomplete="off" {{#isRequired}}required{{/isRequired}} {{#config.isView}}disabled{{/config.isView}} {{^isEditable}}disabled{{/isEditable}}></textarea></div>
