...
class WithdrawnStockItem (Record) :
	...
	stockInTime = DateTimeColumn(input=DateTimeInput(label="เข้าเวลา", order="3.0", isTable=True, isSearch=False, isRequired=False))

	stockOut = IntegerColumn(default=-1, foreignKey="StockOut.id")
	stockOutBy = IntegerColumn(default=-1, foreignKey="User.id")
	stockOutTime = DateTimeColumn(input=DateTimeInput(label="ออกวลา", order="4.0", isTable=True, isSearch=False, isRequired=False))